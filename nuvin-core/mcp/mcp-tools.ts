...
import type { CoreMCPClient, MCPToolSchema } from './mcp-client';
...
function flattenMcpContent(content: any[] | undefined): { type: 'text' | 'json'; value: string | object } {
  if (!content || content.length === 0) return { type: 'text', value: '' };
  const allText = content.every((c) => c && c.type === 'text' && typeof c.text === 'string');
  if (allText) return { type: 'text', value: content.map((c: any) => c.text).join('\n') };
...
export class MCPToolPort implements ToolPort {
  ...
  constructor(private client: CoreMCPClient, opts?: { prefix?: string }) {
